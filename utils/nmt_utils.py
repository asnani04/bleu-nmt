  ...
  if ref_file and tf.gfile.Exists(trans_file):
    for metric in metrics:
      score = evaluation_utils.evaluate(
          ...
          bpe_delimiter=bpe_delimiter)
      if metric == 'bleu':
        evaluation_scores[metric] = score[0]
        utils.print_out("  %s %s: %.1f" % (metric, name, score[0]))
        for i in range(4):
          evaluation_scores['p' + str(i+1)] = score[i+1]
          utils.print_out("  %s %s: %.1f" % ('p' + str(i+1), name, score[i+1]))
      else:
