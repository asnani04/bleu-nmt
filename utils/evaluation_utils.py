...
def evaluate(ref_file, trans_file, metric, bpe_delimiter=None):
  ...
  if metric.lower() == "bleu":
    evaluation_score = _bleu_complete(ref_file, trans_file,
...
def _bleu(ref_file, trans_file, bpe_delimiter=None):
  ...
  bleu_score, p, _, _, _, _ = bleu.compute_bleu(
      per_segment_references, translations, max_order, smooth)
  return 100 * bleu_score

# Experiments

def _bleu_complete(ref_file, trans_file, bpe_delimiter=None):
  """Compute BLEU scores and handling BPE."""
  max_order = 4
  smooth = False
  ...
  bleu_score, p, _, _, _, _ = bleu.compute_bleu(
      per_segment_references, translations, max_order, smooth)
  return 100 * bleu_score, p[0], p[1], p[2], p[3]
