...
def _format_results(name, ppl, scores, metrics):
  ...
  if scores:
    for metric in metrics:
      if metric == 'bleu':
        for i in range(4):
          result_str += ", %s %s %.1f" % (name, 'p' + str(i+1), scores['p' + str(i+1)])
...
def _external_eval(model, global_step, sess, hparams, iterator,
  ...
  if decode:
    for metric in hparams.metrics:
      ...
      if metric == 'bleu':
        for i in range(4):
          sub_metric = 'p' + str(i+1) 
          utils.add_summary(summary_writer, global_step, "%s_%s" % (label, sub_metric), scores[sub_metric])
          # metric: larger is better
          try:
            if save_on_best and scores[sub_metric] > getattr(hparams, "best_" + sub_metric):
              setattr(hparams, "best_" + sub_metric, scores[sub_metric])
              model.saver.save(
                sess,
                os.path.join(
                  getattr(hparams, "best_" + sub_metric + "_dir"), "translate.ckpt"),
                global_step=model.global_step)
          except:
            pass
