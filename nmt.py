  ...
  for metric in hparams.metrics:
    ...
    if metric == 'bleu':
      for i in range(4):
        ...
        hparams.add_hparam("best_" + sub_metric, 0)  # larger is better
        best_metric_dir = os.path.join(hparams.out_dir, "best_" + sub_metric)
        hparams.add_hparam("best_" + sub_metric + "_dir", best_metric_dir)
        tf.gfile.MakeDirs(best_metric_dir)
  ...
  if flags.inference_input_file:
    ...
    if ref_file and tf.gfile.Exists(trans_file):
      for metric in hparams.metrics:
        ...
        if metric == 'bleu':
          utils.print_out("  %s: %.1f" % (metric, score[0]))
          for i in range(4):
            utils.print_out("  %s: %.1f" % ('p' + str(i+1), score[i+1]))
